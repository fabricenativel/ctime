...
node* make_circle(unsigned long long int val)
{
...
node* remove_node(node *current_node, int offset, unsigned long long int* points)
{
    for (int i=0;i<offset;i++)
    {   
...
{
    unsigned long long int start_value = current_node->value; // Que des valeurs différents on arrête l'affichage si on rencontre la même valeur
    printf("(%llu) ",start_value);
    while ((current_node->next)->value != start_value)
    {
        current_node = current_node->next;
        printf("%llu ",current_node->value);
    }
    printf("\n");
}
...
node* play_game(node* circle,int nb_players,unsigned long long int nb_marbles, unsigned long long int* max_points)
{
    int current_player = 1;
    unsigned long long int points[nb_players+1];
    unsigned long long int inc  = 0;
    for (int i=1;i<nb_players+1;i++)
    {
        points[i] = 0;
    }
    for (unsigned long long int i=1; i<nb_marbles+1; i++)
    {   
...
unsigned long long int solve(bool simple)
{
    char* filename = get_filename(simple);
    FILE* fileptr = fopen(filename,"r");
    node* circle;
    unsigned long long int max_pts =0, nb_marbles;
    int nb_players;
    fscanf(fileptr,"%d players; last marble is worth %llu points",&nb_players,&nb_marbles);
    circle = make_circle(0);
    circle = play_game(circle,nb_players,nb_marbles*100,&max_pts);
    return max_pts;
}
...
int main(int argc, char *argv[])
{
    ...
    start();
    simple = (argc >= 2);
    unsigned long long int rep = solve(simple);
    printf("Réponse : %llu \n", rep);
}