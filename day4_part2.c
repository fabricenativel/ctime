...
int getmax(int tab[], int tab_size)
{
    int current_max =0;
    for (int i=0;i<tab_size;i++)
    {
...
int solve() {
    ...
    opti_guard = 0;
    opti_min = indexmax(guards[0],60);
    for (int g=1;g<gnumber;g++)
    {
        if (guards[g][indexmax(guards[g],60)]>guards[opti_guard][opti_min])
        {
            opti_guard = g;
            opti_min = indexmax(guards[g],60);
        }
    }
    return guardnumber[opti_guard]*opti_min;
}
